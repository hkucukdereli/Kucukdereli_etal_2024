...
def calcPrefNulldist(df, nshuffles, condition=None):
    """
    Calculate the null distribution of preferences on each day for a given locomotion state. 
    Conditions can be `dwell`, `explore`, `transition` or None for any locomotion state.
    ...
    pref_list = []
    for n in range(nshuffles):
        stim_t = all_trials.sample(n=len(stim), replace=False).sum()
        neutral_t = all_trials.sample(n=len(neutral), replace=False).sum()
